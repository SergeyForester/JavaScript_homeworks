...
'use strict';
let rub = null;
let userCash = +prompt('Введите кол-во денег')

function declencion(arg = userCash) {

	if (userCash % 10 === 1) {
		rub = 'рубль'
		return rub;	
	} else if(userCash % 10 === 0) {
		rub = 'рублей';
		return rub;
	}else if ( userCash % 2 === 0) {
		rub = 'рубля';
		return rub;
	}
}

alert(`Ваша сумма в ${userCash} ` + declencion(userCash) + ` успешно зачислена.`);
